...
class amr2subgraph:
    ...
    def split_amr(self, raw_amr, contents):
        '''
        ...
        if not raw_amr:
            ...
            if raw_amr[0] == '(':
                ...
                for i in contents:
                    i.append(raw_amr[0])
                self.amr_contents.append(''.join(contents[-1]))
                contents.pop(-1)
            else:
                for i in contents:
                    i.append(raw_amr[0])
    ...
    def amr_reader(self, raw_amr, writer):
        '''
        ...
        for subtree in self.amr_contents:
           # print(subtree)
            key = re.search('\((\S+)',subtree).group(1)
            self.dic[key] = subtree


        for i in self.amr_contents:
            ...
            if e.is_entity:
                ...
                if len(e.next_nodes) > 1:
                    for a in e.next_nodes:
                        try:
                            self.dic[a.name] = e.original_content[:-1].strip()+'\n\t' + a.edge_label+' '+self.dic[a.name]+')'
                            temp.add(a.name)
                        except KeyError:
                            print(raw_amr)
                            print(a.name)
    ...
    def process(filename):
        f = open(filename+'.all.basic-abt-brown-verb.parsed','r')   #path of original text
        g = open('./'+filename.split('/')[-1]+'.sbg','w')    #path to current subgraph path
        raw_amrs = re.split("\n\n",f.read().strip())
        converter = amr2subgraph()
        ...
        for raw_amr in raw_amrs:
            ...
            if not raw_amr:
                continue
            amr_nodes_acronym, path = converter.amr_reader(raw_amr,g)
            #return amr_nodes_acronym, path
        f.close()
        g.close()
